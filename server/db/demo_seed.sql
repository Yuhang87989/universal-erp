...
DELETE FROM stock_out_items WHERE stock_out_id IN (SELECT id FROM stock_out_orders WHERE tenant_id IN (2,3,4,5));
DELETE FROM stock_out_orders WHERE tenant_id IN (2,3,4,5);
DELETE FROM stock_in_items WHERE stock_in_id IN (SELECT id FROM stock_in_orders WHERE tenant_id IN (2,3,4,5));
DELETE FROM stock_in_orders WHERE tenant_id IN (2,3,4,5);
DELETE FROM payment_channels WHERE tenant_id IN (2,3,4,5);
DELETE FROM payment_transactions WHERE tenant_id IN (2,3,4,5);
DELETE FROM stock_alerts WHERE tenant_id IN (2,3,4,5);
DELETE FROM voucher_items WHERE voucher_id IN (SELECT v.id FROM vouchers v JOIN accounting_books b ON v.book_id=b.id WHERE b.tenant_id IN (2,3,4,5));
DELETE FROM voucher_seals WHERE voucher_id IN (SELECT v.id FROM vouchers v JOIN accounting_books b ON v.book_id=b.id WHERE b.tenant_id IN (2,3,4,5));
...
DELETE FROM stocktake_items WHERE stocktake_id IN (SELECT id FROM stocktakes WHERE tenant_id IN (2,3,4,5));
DELETE FROM stocktakes WHERE tenant_id IN (2,3,4,5);
DELETE FROM accounting_books WHERE tenant_id IN (2,3,4,5);
DELETE FROM finance_records WHERE tenant_id IN (2,3,4,5);
DELETE FROM sale_items WHERE sales_order_id IN (SELECT id FROM sales_orders WHERE tenant_id IN (2,3,4,5));
DELETE FROM sales_orders WHERE tenant_id IN (2,3,4,5);
DELETE FROM purchase_items WHERE purchase_order_id IN (SELECT id FROM purchase_orders WHERE tenant_id IN (2,3,4,5));
...
-- ---------- 2. 用户（每账套一个owner） ----------
INSERT IGNORE INTO users (tenant_id, username, password_hash, real_name, role, phone) VALUES
...
-- ---------- 3. 分类 ----------
INSERT IGNORE INTO categories (tenant_id, name, sort_order) VALUES
...
-- ---------- 4. 商品 ----------
-- 账套2 生鲜
INSERT IGNORE INTO products (tenant_id, category_id, name, barcode, unit, cost_price, sell_price, min_stock, status)
...
-- ---------- 5. 仓库 ----------
INSERT IGNORE INTO warehouses (tenant_id, code, name, address, manager, is_default, sort_order) VALUES
...
-- ---------- 6. 供应商 ----------
INSERT IGNORE INTO suppliers (tenant_id, name, contact_name, phone, address, bank_name, status) VALUES
...
-- ---------- 7. 客户/会员 ----------
INSERT IGNORE INTO customers (tenant_id, name, phone, gender, level, points) VALUES
...
-- 账套2 默认仓=WH002
INSERT IGNORE INTO inventory (tenant_id, product_id, warehouse_id, quantity)
...
INSERT IGNORE INTO purchase_orders (tenant_id, order_no, supplier_id, total_amount, status, order_date)
VALUES (2,'PO20260810001',1,760.00,'received','2026-08-10');
INSERT IGNORE INTO purchase_items (purchase_order_id, product_id, quantity, unit_cost, received_quantity)
SELECT po.id, p.id,
...
-- 账套2 生鲜：5笔POS销售
INSERT IGNORE INTO sales_orders (tenant_id, order_no, order_type, customer_id, total_amount, discount_amount, actual_amount, paid_amount, payment_method, status, order_date) VALUES
...
INSERT IGNORE INTO sale_items (sales_order_id, product_id, quantity, unit_price)
SELECT so.id, p.id,
  CASE so.order_no
...
-- 账套3 数码
INSERT IGNORE INTO sales_orders (tenant_id, order_no, order_type, customer_id, total_amount, discount_amount, actual_amount, paid_amount, payment_method, status, platform, order_date) VALUES
...
-- ---------- 11. 财务收支 ----------
INSERT IGNORE INTO finance_records (tenant_id, type, category, amount, payment_method, remark, record_date) VALUES
...
INSERT IGNORE INTO accounting_books (id, tenant_id, book_name, entity_name, credit_code, entity_type) VALUES
...
-- 一级科目
INSERT IGNORE INTO accounting_accounts (book_id, code, name, category, direction, level, parent_id)
...
-- ---------- 13. 印章（每账套3枚备案印章） ----------
INSERT IGNORE INTO seals (book_id, seal_type, seal_name, seal_code, is_filed, is_active)
...
INSERT IGNORE INTO vouchers (book_id, voucher_type, voucher_no, voucher_date, total_debit, total_credit, is_balanced, status, remark)
...
INSERT IGNORE INTO voucher_items (voucher_id, line_no, account_id, summary, debit_amount, credit_amount)
SELECT v.id, 1,
...
-- ---------- 15. 库存预警 ----------
INSERT IGNORE INTO stock_alerts (tenant_id, alert_type, product_id, warehouse_id, threshold_value, current_value, alert_level, status, message)
SELECT p.tenant_id,
...
-- ---------- 16. 支付渠道默认配置（每账套4种） ----------
INSERT IGNORE INTO payment_channels (tenant_id, channel_code, channel_name, channel_type, is_enabled, is_default, sort_order)
...
INSERT IGNORE INTO stock_in_orders (tenant_id, order_no, warehouse_id, in_type, supplier_id, source_order_type, source_order_id, total_amount, status, confirm_time, remark)
...
INSERT IGNORE INTO stock_in_items (stock_in_id, product_id, quantity, unit_cost)
SELECT sio.id, p.id,
  CASE p.name WHEN '大白菜' THEN 200 WHEN '土豆' THEN 150 WHEN '番茄' THEN 100 END,
  CASE p.name WHEN '大白菜' THEN 1.20 WHEN '土豆' THEN 1.80 WHEN '番茄' THEN 2.50 END
...
INSERT IGNORE INTO stock_in_orders (tenant_id, order_no, warehouse_id, in_type, supplier_id, total_amount, status, confirm_time, remark)
...
-- ---------- 18. 独立出库单（仓库管理模块演示数据） ----------
-- 账套2：销售出库
INSERT IGNORE INTO stock_out_orders (tenant_id, order_no, warehouse_id, out_type, customer_id, total_amount, status, confirm_time, remark)
...
INSERT IGNORE INTO stock_out_items (stock_out_id, product_id, quantity, unit_cost)
SELECT soo.id, p.id,
...
-- 账套5：报损出库
INSERT IGNORE INTO stock_out_orders (tenant_id, order_no, warehouse_id, out_type, total_amount, status, confirm_time, remark)
...
INSERT IGNORE INTO stock_transfers (tenant_id, transfer_no, from_warehouse_id, to_warehouse_id, status, total_amount, confirm_time, remark)
...
INSERT IGNORE INTO stock_transfer_items (transfer_id, product_id, quantity, unit_cost)
SELECT st.id, p.id,
...
-- 期初库存流水
INSERT IGNORE INTO inventory_logs (tenant_id, product_id, warehouse_id, change_type, quantity, before_quantity, after_quantity, unit_cost, remark)
...
-- 入库流水
INSERT IGNORE INTO inventory_logs (tenant_id, product_id, warehouse_id, change_type, quantity, before_quantity, after_quantity, unit_cost, reference_type, reference_id, remark)
